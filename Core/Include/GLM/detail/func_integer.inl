...
namespace detail
{
	...
	struct compute_findLSB
	{
		GLM_FUNC_QUALIFIER static int call(genIUType Value)
		{
			...
			return Core::bitCount(~Value & (Value - static_cast<genIUType>(1)));
		}
	};
...
#	endif//GLM_HAS_BITSCAN_WINDOWS
	...
	struct compute_findMSB_vec
	{
		GLM_FUNC_QUALIFIER static vec<L, int, Q> call(vec<L, T, Q> const& v)
		{
			...
			return vec<L, int, Q>(sizeof(T) * 8 - 1) - Core::bitCount(~x);
		}
	};
...
#	endif//GLM_HAS_BITSCAN_WINDOWS
	...
	{
		return bitfieldReverse(Core::vec<1, genType, Core::defaultp>(x)).x;
	}
	...
	{
		return bitCount(Core::vec<1, genType, Core::defaultp>(x)).x;
	}
