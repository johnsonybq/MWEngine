...
#	endif
	...
	//	vec<3, valType> c = Core::cos(eulerAngle * valType(0.5));
	//	vec<3, valType> s = Core::sin(eulerAngle * valType(0.5));
	//
	...
	{
		vec<3, T, Q> c = Core::cos(eulerAngle * T(0.5));
		vec<3, T, Q> s = Core::sin(eulerAngle * T(0.5));
...
#	endif
	...
	GLM_FUNC_QUALIFIER vec<3, T, Q> operator*(qua<T, Q> const& q, vec<3, T, Q> const& v)
	{
		...
		vec<3, T, Q> const uv(Core::cross(QuatVector, v));
		vec<3, T, Q> const uuv(Core::cross(QuatVector, uv));
