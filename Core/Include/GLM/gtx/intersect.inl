...
namespace Core
{
	...
	{
		typename genType::value_type d = Core::dot(dir, planeNormal);
		typename genType::value_type Epsilon = std::numeric_limits<typename genType::value_type>::epsilon();

		if(d < -Epsilon)
		{
			intersectionDistance = Core::dot(planeOrig - orig, planeNormal) / d;
			return true;
		}
	...
	)
	{
		...
		vec<3, T, Q> const p = Core::cross(dir, edge2);

		// if determinant is near zero, ray lies in plane of triangle
		T const det = Core::dot(edge1, p);
		...
		if(det > std::numeric_limits<T>::epsilon())
		{
			...
			baryPosition.x = Core::dot(dist, p);
			if(baryPosition.x < static_cast<T>(0) || baryPosition.x > det)
				return false;

			// prepare to test V parameter
			Perpendicular = Core::cross(dist, edge1);

			// calculate V parameter and test bounds
			baryPosition.y = Core::dot(dir, Perpendicular);
		...
		distance = Core::dot(edge2, Perpendicular) * inv_det;
